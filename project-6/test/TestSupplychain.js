...
contract('SupplyChain', function(accounts) {
    ...
    const productPrice = web3.toWei(0.000000000000000000001, "ether")
    var itemState = 0
    ...
    // Set roles
    it("Testing everyone has asigned its role", async() => {
        ...
        assert.equal(await supplyChain.isFarmer(originFarmerID), true, 'Error: Missing or Invalid role')
        assert.equal(await supplyChain.isDistributor(distributorID), true, 'Error: Missing or Invalid role')
        assert.equal(await supplyChain.isRetailer(retailerID), true, 'Error: Missing or Invalid role')
        assert.equal(await supplyChain.isConsumer(consumerID), true, 'Error: Missing or Invalid role')
    })
    ...
    // 6th Test
    it("Testing smart contract function shipItem() that allows a distributor to ship cocoa", async() => {
        const supplyChain = await SupplyChain.deployed()
    ...
    it("Testing smart contract function fetchItemBufferTwo() that allows anyone to fetch item details from blockchain", async() => {
        ...
        assert.equal(resultBufferTwo[2], productID, 'Missing or Invalid productID')
        assert.equal(resultBufferTwo[3], productNotes, 'Missing or Invalid productNotes')
        assert.equal(resultBufferTwo[4], productPrice, 'Error: Invalid item productPrice')
        assert.equal(resultBufferTwo[5].toNumber(), 7, 'Missing or Invalid state')
        assert.equal(resultBufferTwo[6], distributorID, 'Missing or Invalid distributorID')
        assert.equal(resultBufferTwo[7], retailerID, 'Missing or Invalid retailerID')        
        assert.equal(resultBufferTwo[8], consumerID, 'Missing or Invalid consumerID')        
    })
